...
namespace JerryPlat.Pay.WxPay
{
    ...
    public class JsApiPay
    {
	    ...
	    */
        ...
        public WxPayData GetUnifiedOrderResult(UnifiedOrder unifiedOrder)
        {
            //统一下单
            WxPayData data = new WxPayData();
            data.SetValue("body", "邦邦学车-在线报名");
            data.SetValue("detail", unifiedOrder.detail); //商品详情
            data.SetValue("out_trade_no", unifiedOrder.OrderNo);
            data.SetValue("total_fee", (Math.Round(unifiedOrder.Amount * 100, 0)).ToString());//微信支付提交的金额是不能带小数点的，且是以分为单位，所以我们系统如果是以元为单位要处理下金额，即先乘以100，再去小数点
